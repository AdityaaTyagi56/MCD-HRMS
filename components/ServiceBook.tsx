    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Immutable Blockchain-verified Career Timeline</p>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm text-center">
                ...
                <div className="mt-8 pt-6 border-t border-gray-200 text-left space-y-4">
                    <div>
                        <p className="text-xs text-gray-400 uppercase font-semibold mb-1">Joining Date</p>
                        <p className="text-sm font-medium text-gray-900 flex items-center gap-2">
                            <Calendar size={14} /> {employee.joiningDate}
                        </p>
                    </div>
                    <div>
                        <p className="text-xs text-gray-400 uppercase font-semibold mb-1">Retirement Date</p>
                        <p className="text-sm font-medium text-gray-900 flex items-center gap-2">
                    ...
                    {employee.serviceBook.map((record, idx) => (
                        <div key={record.id} className="relative pl-8">
                            <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-white shadow-sm ${
                                record.type === 'Hiring' ? 'bg-green-500' :
                            ...
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                                <div>
                                    <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide mb-1 ${
                                        record.type === 'Hiring' ? 'bg-green-100 text-green-700' :
                                    ...
                                    <p className="text-sm text-gray-500 mt-1">Authorized by: {record.authority}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-medium text-gray-900">{record.date}</p>
                                    <p className="text-[10px] font-mono text-gray-400 mt-1 truncate w-24" title={record.hash}>
                                        Hash: {record.hash}
                                    </p>
                                </div>
                            </div>
