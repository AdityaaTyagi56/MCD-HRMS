    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">Algorithmic posting based on tenure policy (3 Years)</p>
        </div>

        <div className="flex bg-white border border-gray-200 rounded-xl p-1">
          <button
            onClick={() => setFilter('Due')}
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${filter === 'Due'
      ...
      <div className="grid gap-4">
        {filteredEmployees.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
            ...
            <p className="text-gray-700">All employees are within their tenure limits.</p>
          </div>
        ) : (
              ...
              <div key={emp.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div className="flex items-center gap-8">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-gray-500">Since {emp.currentPostingDate}</p>
                  </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div>
                ...
                <p className="text-sm text-gray-600">Finding suitable vacancies for {selectedEmployee.name}</p>
              </div>
              <button
                onClick={closeModal}
            ...
            {isSearching && (
              <div className="p-12 text-center">
                ...
                <p className="text-gray-600 text-sm mt-2">AI is analyzing department needs, location preferences, and skill match</p>
              </div>
            )}
              ...
              <div className="p-6">
                ...
                <div className="space-y-3">
                  {suggestedVacancies.map((vacancy, idx) => (
                    <div
                      key={vacancy.id}
                      className={`p-4 rounded-xl border-2 transition-all ${transferInitiated === vacancy.id
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${idx === 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                            }`}>
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${transferInitiated === vacancy.id
